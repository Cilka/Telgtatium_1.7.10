package com.hexopygate.Telgttatium.buildingblocks.glass.fullglasspanes;

import com.hexopygate.Telgttatium.blockarchtype.TelNormalGlassPane;

public class Magenta_Glass_Pane extends TelNormalGlassPane {

	public Magenta_Glass_Pane(String texture) {
		super(texture);
		// TODO Auto-generated constructor stub
	}
public Magenta_Glass_Pane(String faceTex, String trimTex) {
		super(faceTex,trimTex);
		// TODO Auto-generated constructor stub
	}

}
