...
public class TelChunkGenerator implements  IChunkProvider{
	/** RNG. */
	private Random rand;
	...
	public TelChunkGenerator(World world, long seed, boolean mapFeaturesEnabled)
	{
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									blocks[j3 += short1] = Blocks.clay;//these can be set to custom blocks
								} else if (k2 * 8 + l2 < b0) {
	...
	public void replaceBlocksForBiome(int par1, int par2, Block[] blocks, byte[] par3ArrayOfByte, BiomeGenBase[] par4ArrayOfBiomeGenBase) {
		...
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				TelBiomeGenBase biomegenbase = (TelBiomeGenBase) par4ArrayOfBiomeGenBase[l + k * 16];
